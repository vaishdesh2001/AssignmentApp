...
public class FrontEndInterface {
  ...
  /**
   * Adds the assignment once the user has inputed its attributes correctly. It
   * invokes the Assignment's constructors to create and store the it in the schedule.
   * 
   * @param schedule
   */
  private static void addAssignment(AssignmentScheduler schedule) {
    System.out.println("Enter the due date (MM/DD/YYYY), the name of the assignment, the course name"
        + "and the number of points it is worth.");
    System.out.println("Please enter each attribute on a new line.");

    String dateStr = sc.nextLine();
    String[] dateArr = dateStr.split("/");
    Date dueDate = new Date(Integer.parseInt(dateArr[2]), Integer.parseInt(dateArr[1]), 
        Integer.parseInt(dateArr[0])); // Date (year, month, date)
 ...
 /* private static Product createProduct(String name, String type, String manufacturer, Long barcode,
   ...
   * @param schedule
   */
  private static void removeAssigment(AssignmentScheduler schedule) {
    ...
    try {
      AssignmentScheduler.displayGet(schedule.get(dueDate));
    ...
    System.out.println();
     ...
     System.out.println("The assignment you were trying to remove did not exist in the inventory.");
    }
  }
  ...
  private static boolean assignmentStored(AssignmentScheduler schedule) {
    System.out.println("Enter the due date (MM/DD/YYYY), the name of the assignment, the course name"
        + "and the number of points it is worth to check if it is stored in the inventory.");
    System.out.println("Please enter each attribute on a new line.");
    ...
    Assignment userAssignment = new Assignment(dueDate, assnName, courseName, points);
    if (schedule.contains(userAssignment)) {
      System.out.println("The assignment is stored in the inventory.");
      return true;
    }
    System.out.println("The assignment could not be found in the inventory.");
    return false;
  }

  /**
   * Finds the assignment associated with the date inputed in the method, if it exists 
   * in the schedule. Once found it displays the attributes of the desired assignment. 
   * 
   * @param schedule
   */
  private static void findAssignment(AssignmentScheduler schedule) {
    ...
    try {
      ...
      System.out.println(
          "The assignment you were trying to remove could not be found in the schedule.");
      return;
    }
  }
  ...
  public static void loadFromFile(AssignmentScheduler schedule) {
    File file = new File("src/assignmentToDo.txt");
    ...
    try (Scanner sc = new Scanner(file)) {
      ...
      if (sc.nextLine().equals("Due Date, Assignment Name, Course Name, Number of Points")) {
        while (sc.hasNextLine()) {
          ...
          System.out.println(assnName + " " + courseName);
          if (schedule.add(new Assignment(dueDate, assnName, courseName, points))) {
            continue;
          } else {
  ...
  /**
   * Prints the assignments in the schedule in order, similar to how a Linked List or an 
   * Array prints the elements.
   * 
   * @param schedule
   */
  ...
  private static void menu() {
    ...
    System.out.println("Enter 'A' to add the an assignment into the schedule.");
